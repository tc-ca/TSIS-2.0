...
function showHideMatchConfirmed(eContext) {
  ...
  var isPresent = formContext.getAttribute('ppp_ispresent').getValue();
  var haveProceeded = formContext
    .getAttribute('ppp_matchfoundconfirmation')
    .getValue();
...
function toggleDisabledAllControls(
  eContext,
  disable,
  keepLockedList,
  keepUnlockedList
...
function showHideFlightConnections(
  eContext,
  connectionCountName,
  maxConnections
) {
  var formContext = eContext.getFormContext();
  var connectionCount = formContext
    .getAttribute(connectionCountName)
    .getValue();
  for (var i = 1; i <= maxConnections; i++) {
    if (connectionCount >= i) {
      formContext.getControl('ppp_flightconnection' + i).setVisible(true);
      formContext
        .getAttribute('ppp_flightconnection' + i)
      ...
      formContext
        .getControl('ppp_flightconnection' + i + '1')
        .setVisible(false);
...
function flightValidation(eContext, flightNameArray) {
  ...
  for (var x = 0; x < flightNameArray.length; x++) {
    ...
    for (var y = 0; y < flightNameArray.length; y++) {
      ...
      if (
        x != y &&
        flightX.getValue() != null &&
        flightY.getValue() != null &&
        flightX.getValue()[0].id == flightY.getValue()[0].id
      ) {
        if (globalContext.userSettings.languageId == 1033) {
          formContext
            .getControl(flightNameArray[x])
            .setNotification('Duplicate aerodromes not permitted.');
          formContext
            .getControl(flightNameArray[y])
            .setNotification('Duplicate aerodromes not permitted.');
        } else {
          formContext
            .getControl(flightNameArray[x])
            .setNotification('Duplicate aerodromes not permitted. (fr)');
          formContext
            .getControl(flightNameArray[y])
            .setNotification('Duplicate aerodromes not permitted. (fr)');
        }
